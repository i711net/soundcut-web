  ...
  return <div className="app" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); loadFile(e.dataTransfer.files[0]) }}>
    ...
    <main>
      ...
      <section className="workspace">
        ...
        {trackStore.tracks.slice(3).map(track => <div className={`track extra-track ${track.expanded ? 'expanded' : ''}`} key={track.id}><TrackControls track={track} active={trackStore.activeId === track.id} onActivate={() => trackStore.setActiveId(track.id)} onChange={patch => trackStore.updateTrack(track.id, patch)} onDelete={() => trackStore.deleteTrack(track.id)}/><div className="track-canvas">{track.buffer ? <><TrackClipLane track={track} timelineDuration={timelineDuration} tool={timelineTool} selection={selection} onSelection={setSelection} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId(track.id); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (track.playbackRate * track.clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/><ClipControls track={track} onChange={patch => trackStore.updateTrack(track.id, patch)} onApplyPitch={() => applyPitch(track.id)}/></> : <button className="video-empty" onClick={() => requestImport(track.id)}><Upload/><span>导入音频</span></button>}</div></div>)}
