...
import { Download, Upload, MousePointer2, Scissors, Waves, Undo2, Redo2, Play, Pause, SkipBack, SkipForward, ZoomIn, ZoomOut, Music2, Trash2, LockKeyhole, RotateCcw } from 'lucide-react'
...
export default function App() {
  ...
  const loadFile = async (file?: File) => {
    ...
    try {
      ...
      setBuffer(decoded); setFileName(file.name.replace(/\.[^.]+$/, '')); setSelection([0, decoded.duration]); setSettings(s => ({ ...s, start: 0, end: decoded.duration })); setCurrentTime(0); setHistory([]); setFuture([]); setStatus('音频已在本机载入')
  ...
  return <div className="app" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); loadFile(e.dataTransfer.files[0]) }}>
    ...
    <main>
      ...
      <section className="workspace">
        ...
        <div className="privacy"><LockKeyhole/> 音频仅在你的浏览器中处理，不会上传</div>
      </section>
      <aside className="inspector"><div className="panel-title">属性</div><label>音频名称<input value={fileName} onChange={e => setFileName(e.target.value)}/></label><div className="info-row"><span>时长</span><b>{formatTime(buffer?.duration ?? 0, true)}</b></div><hr/><label>音量 <output>{Math.round(settings.gain * 100)}%</output><input type="range" min="0" max="2" step=".01" value={settings.gain} onChange={e => setSettings({...settings, gain: +e.target.value})}/></label><label>淡入 <output>{settings.fadeIn.toFixed(1)}s</output><input type="range" min="0" max="10" step=".1" value={settings.fadeIn} onChange={e => setSettings({...settings, fadeIn: +e.target.value})}/></label><label>淡出 <output>{settings.fadeOut.toFixed(1)}s</output><input type="range" min="0" max="10" step=".1" value={settings.fadeOut} onChange={e => setSettings({...settings, fadeOut: +e.target.value})}/></label><label>播放速度 <output>{speed.toFixed(2)}×</output><input type="range" min=".5" max="2" step=".05" value={speed} onChange={e => setSpeed(+e.target.value)}/></label><hr/><label>导出格式<select><option>WAV · 16 bit</option></select></label><div className="selection-info"><span>当前选区</span><b>{formatTime(selection[0], true)} — {formatTime(selection[1], true)}</b></div></aside>
    </main>
