...
import Waveform, { type WaveformMark } from './Waveform'
import { bufferToWav, formatTime, insertBuffer, moveRange, removeRange, renderBuffer, separateStereo, transformRange, type EditSettings } from './audio'
...
type Snapshot = { trackId: string; buffer: AudioBuffer; selection: [number, number] }
...
export default function App() {
  ...
  const [editMarks, setEditMarks] = useState<Record<string, WaveformMark[]>>({})
  ...
  const toggleRecording = async () => {
    ...
    try {
      ...
      if (window.top !== window.self) throw new Error('当前编辑器运行在内嵌预览窗口中，WebRTC 麦克风会被上层页面拦截。请点击“在新标签页打开”或直接访问 Cloudflare Pages 的 HTTPS 地址。')
      const permissionPolicy = document as Document & { permissionsPolicy?: { allowsFeature: (feature: string) => boolean }; featurePolicy?: { allowsFeature: (feature: string) => boolean } }
      const policy = permissionPolicy.permissionsPolicy || permissionPolicy.featurePolicy
      if (policy && !policy.allowsFeature('microphone')) throw new Error('当前页面的 Permissions Policy 禁止麦克风。请直接打开本站地址，不要在 Cloudflare、GitHub 或其他网站的内嵌预览框中运行。')
      ...
      const blockedByWebRtc = /getUserMedia blocked|webrtc/i.test(detail)
      const message = blockedByWebRtc ? 'WebRTC 麦克风被浏览器或当前预览环境拦截。请直接在 Chrome/Edge 新标签页打开 Cloudflare Pages 的 HTTPS 地址，允许地址栏中的麦克风权限后刷新。不要在内嵌预览窗口中录音。' : name === 'NotAllowedError' || name === 'SecurityError' ? '麦克风权限被拒绝。请点击地址栏左侧的权限图标，把“麦克风”改为“允许”，然后刷新页面。若没有权限选项，请检查 Windows“隐私和安全性 → 麦克风”。' : name === 'NotFoundError' || name === 'DevicesNotFoundError' ? '没有找到可用的麦克风，请连接麦克风后重试。' : name === 'NotReadableError' || name === 'TrackStartError' ? '麦克风正被其他程序占用，或系统禁止浏览器访问。请关闭占用程序并检查 Windows 麦克风隐私设置。' : detail || '无法启动麦克风，请确认浏览器允许本站使用麦克风。'
  ...
  const activeEditBuffer = trackStore.activeId === 'track-1' ? buffer : trackStore.activeTrack.buffer
  const addEditMark = (trackId: string, start: number, end: number, label: string, kind: WaveformMark['kind']) => setEditMarks(items => ({ ...items, [trackId]: [...(items[trackId] || []).slice(-11), { id: `${Date.now()}-${Math.random()}`, start, end, label, kind }] }))
  const setEditedTrackBuffer = (trackId: string, next: AudioBuffer) => {
    mixerPlayback.stop(); stop()
    if (trackId === 'track-1') setBuffer(next)
    else trackStore.setTrackEditedBuffer(trackId, next)
  }
  const remember = () => { if (activeEditBuffer) { setHistory(h => [...h.slice(-29), { trackId: trackStore.activeId, buffer: activeEditBuffer, selection }]); setFuture([]) } }
  const applyActiveEdit = (next: AudioBuffer, message: string, nextSelection: [number, number] = [0, next.duration]) => {
    remember(); setEditedTrackBuffer(trackStore.activeId, next); setSelection(nextSelection); setSettings(s => ({ ...s, start: nextSelection[0], end: nextSelection[1] })); setCurrentTime(Math.min(currentTimeRef.current, next.duration)); setStatus(message)
  }
  const validSelection = () => !!activeEditBuffer && selection[1] - selection[0] >= .01
  const keepSelection = () => { if (!activeEditBuffer || !validSelection()) return; const next = renderBuffer(activeEditBuffer, { start: selection[0], end: selection[1], gain: 1, fadeIn: 0, fadeOut: 0 }); applyActiveEdit(next, '已裁剪：仅保留选区'); setEditMarks(items => ({ ...items, [trackStore.activeId]: [{ id: `${Date.now()}`, start: 0, end: next.duration, label: '裁剪片段', kind: 'clip' }] })) }
  const deleteSelection = () => { if (!activeEditBuffer || !validSelection() || selection[1] - selection[0] >= activeEditBuffer.duration) return; const at = selection[0], next = removeRange(activeEditBuffer, ...selection); applyActiveEdit(next, '已删除选区', [Math.min(at, next.duration), Math.min(at, next.duration)]); addEditMark(trackStore.activeId, Math.min(at, next.duration), Math.min(at, next.duration), '删除接点', 'cut') }
  const copySelection = () => { if (!activeEditBuffer || !validSelection()) return; setAudioClipboard(renderBuffer(activeEditBuffer, { start: selection[0], end: selection[1], gain: 1, fadeIn: 0, fadeOut: 0 })); addEditMark(trackStore.activeId, selection[0], selection[1], '复制源', 'copy'); setStatus(`已复制 ${formatTime(selection[1] - selection[0], true)} 音频`) }
  const cutSelection = () => { if (!activeEditBuffer || !validSelection() || selection[1] - selection[0] >= activeEditBuffer.duration) return; const at = selection[0]; copySelection(); const next = removeRange(activeEditBuffer, ...selection); applyActiveEdit(next, '已剪切选区', [at, at]); addEditMark(trackStore.activeId, at, at, '剪切接点', 'cut') }
  const pasteAudio = () => { if (!activeEditBuffer || !audioClipboard) return; const at = Math.min(currentTimeRef.current, activeEditBuffer.duration), next = insertBuffer(activeEditBuffer, audioClipboard, at); applyActiveEdit(next, '已在播放头位置粘贴音频', [at, at + audioClipboard.duration]); addEditMark(trackStore.activeId, at, at + audioClipboard.duration, '粘贴片段', 'paste') }
  const moveActiveSelection = (from: number, to: number, newStart: number) => {
    if (!activeEditBuffer) return
    const length = to - from, delta = newStart - from, trackId = trackStore.activeId, next = moveRange(activeEditBuffer, from, to, newStart)
    applyActiveEdit(next, `音频片段已移动到 ${formatTime(newStart, true)}`, [newStart, newStart + length])
    setEditMarks(items => ({ ...items, [trackId]: (items[trackId] || []).map(mark => mark.start >= from && mark.end <= to ? { ...mark, start: mark.start + delta, end: mark.end + delta } : mark).concat({ id: `${Date.now()}`, start: newStart, end: newStart + length, label: '已移动片段', kind: 'clip' }) }))
  }
  const transformSelection = (mode: 'silence' | 'reverse' | 'normalize' | 'fadeIn' | 'fadeOut', label: string) => { if (!activeEditBuffer || !validSelection()) return; applyActiveEdit(transformRange(activeEditBuffer, ...selection, mode), `已应用：${label}`, selection) }
  const splitAtPlayhead = () => {
    if (!activeEditBuffer) return
    const at = currentTimeRef.current
    if (at <= .01 || at >= activeEditBuffer.duration - .01) return setStatus('请把播放头放在音频中间再分割')
    const left = renderBuffer(activeEditBuffer, { start: 0, end: at, gain: 1, fadeIn: 0, fadeOut: 0 }), right = renderBuffer(activeEditBuffer, { start: at, end: activeEditBuffer.duration, gain: 1, fadeIn: 0, fadeOut: 0 })
    const sourceId = trackStore.activeId; remember(); setEditedTrackBuffer(sourceId, left); const newId = trackStore.addTrack(right, `${trackStore.activeTrack.name} · 后半段`); addEditMark(sourceId, Math.max(0, left.duration - .001), left.duration, '分割点', 'split'); addEditMark(newId, 0, right.duration, '分割片段 2', 'clip'); setSelection([0, right.duration]); setCurrentTime(0); setStatus('已在播放头处分割，边界已标记，后半段已放入新轨道')
  }
  const duplicateSelectionToTrack = () => { if (!activeEditBuffer || !validSelection()) return; const sourceId = trackStore.activeId, clip = renderBuffer(activeEditBuffer, { start: selection[0], end: selection[1], gain: 1, fadeIn: 0, fadeOut: 0 }); addEditMark(sourceId, selection[0], selection[1], '复制源', 'copy'); const newId = trackStore.addTrack(clip, `${trackStore.activeTrack.name} · 片段`); addEditMark(newId, 0, clip.duration, '复制片段', 'clip'); setSelection([0, clip.duration]); setStatus('选区已复制为新音轨，来源与新片段均已标记') }
  ...
  const importAudioFragment = async (file?: File) => {
    if (!file) return
    try { const ctx = audioContext.current ?? new AudioContext(); audioContext.current = ctx; const decoded = await ctx.decodeAudioData(await file.arrayBuffer()); if (!activeEditBuffer) { const id = trackStore.addTrack(decoded, file.name.replace(/\.[^.]+$/, '')); trackStore.setActiveId(id); addEditMark(id, 0, decoded.duration, '导入片段', 'clip'); setSelection([0, decoded.duration]); setStatus('片段已导入新音轨'); return } const targetId = trackStore.activeId, at = Math.min(currentTimeRef.current, activeEditBuffer.duration), next = insertBuffer(activeEditBuffer, decoded, at); applyActiveEdit(next, '片段已插入播放头位置', [at, at + decoded.duration]); addEditMark(targetId, at, at + decoded.duration, '导入片段', 'paste') } catch { setStatus('无法导入该音频片段') }
  }
  const restoreSnapshot = (snapshot: Snapshot) => { trackStore.setActiveId(snapshot.trackId); setEditedTrackBuffer(snapshot.trackId, snapshot.buffer); setSelection(snapshot.selection); setSettings(s => ({ ...s, start: snapshot.selection[0], end: snapshot.selection[1] })) }
  const undo = () => { const last = history.at(-1); if (!last) return; const currentTrack = trackStore.tracks.find(track => track.id === last.trackId), currentBuffer = last.trackId === 'track-1' ? buffer : currentTrack?.buffer; if (!currentBuffer) return; setFuture(f => [{ trackId: last.trackId, buffer: currentBuffer, selection }, ...f]); restoreSnapshot(last); setHistory(h => h.slice(0, -1)); setStatus('已撤销上一步剪辑') }
  const redo = () => { const next = future[0]; if (!next) return; const currentTrack = trackStore.tracks.find(track => track.id === next.trackId), currentBuffer = next.trackId === 'track-1' ? buffer : currentTrack?.buffer; if (!currentBuffer) return; setHistory(h => [...h, { trackId: next.trackId, buffer: currentBuffer, selection }]); restoreSnapshot(next); setFuture(f => f.slice(1)); setStatus('已重做剪辑') }
