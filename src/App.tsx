...
export default function App() {
  ...
  const keepSelection = () => { if (!activeEditBuffer || !validSelection()) return; const next = renderBuffer(activeEditBuffer, { start: selection[0], end: selection[1], gain: 1, fadeIn: 0, fadeOut: 0 }); applyActiveEdit(next, '已裁剪：仅保留选区') }
  const deleteSelection = () => { if (!activeEditBuffer || !validSelection() || selection[1] - selection[0] >= activeEditBuffer.duration) return; const next = removeRange(activeEditBuffer, ...selection); applyActiveEdit(next, '已删除选区', [Math.min(selection[0], next.duration), Math.min(selection[0], next.duration)]) }
  const copySelection = () => { if (!activeEditBuffer || !validSelection()) return; setAudioClipboard(renderBuffer(activeEditBuffer, { start: selection[0], end: selection[1], gain: 1, fadeIn: 0, fadeOut: 0 })); setStatus(`已复制 ${formatTime(selection[1] - selection[0], true)} 音频`) }
  const cutSelection = () => { if (!activeEditBuffer || !validSelection() || selection[1] - selection[0] >= activeEditBuffer.duration) return; copySelection(); const next = removeRange(activeEditBuffer, ...selection); applyActiveEdit(next, '已剪切选区', [selection[0], selection[0]]) }
  const pasteAudio = () => { if (!activeEditBuffer || !audioClipboard) return; const at = Math.min(currentTimeRef.current, activeEditBuffer.duration), next = insertBuffer(activeEditBuffer, audioClipboard, at); applyActiveEdit(next, '已在播放头位置粘贴音频', [at, at + audioClipboard.duration]) }
  ...
  const splitAtPlayhead = () => {
    ...
    remember(); setEditedTrackBuffer(trackStore.activeId, left); trackStore.addTrack(right, `${trackStore.activeTrack.name} · 后半段`); setSelection([0, right.duration]); setCurrentTime(0); setStatus('已在播放头处分割，后半段已放入新轨道')
  }
  const duplicateSelectionToTrack = () => { if (!activeEditBuffer || !validSelection()) return; const clip = renderBuffer(activeEditBuffer, { start: selection[0], end: selection[1], gain: 1, fadeIn: 0, fadeOut: 0 }); trackStore.addTrack(clip, `${trackStore.activeTrack.name} · 片段`); setSelection([0, clip.duration]); setStatus('选区已复制为新音轨') }
  ...
  const importAudioFragment = async (file?: File) => {
    if (!file) return
    try { const ctx = audioContext.current ?? new AudioContext(); audioContext.current = ctx; const decoded = await ctx.decodeAudioData(await file.arrayBuffer()); if (!activeEditBuffer) { const id = trackStore.addTrack(decoded, file.name.replace(/\.[^.]+$/, '')); trackStore.setActiveId(id); setSelection([0, decoded.duration]); setStatus('片段已导入新音轨'); return } const at = Math.min(currentTimeRef.current, activeEditBuffer.duration), next = insertBuffer(activeEditBuffer, decoded, at); applyActiveEdit(next, '片段已插入播放头位置', [at, at + decoded.duration]) } catch { setStatus('无法导入该音频片段') }
  }
