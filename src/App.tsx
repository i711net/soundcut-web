...
export default function App() {
  ...
  const keepSelection = () => { if (!selectedClip) return setStatus('请先点击需要裁剪的片段'); setStatus('当前片段已经是独立非破坏式区域；可拖动两端调整入点和出点') }
  const deleteSelection = () => { if (!selectedClip) return setStatus('请先点击需要删除的音频片段'); remember(); trackStore.deleteClip(trackStore.activeId, selectedClip.id); setSelectedClipId(''); setStatus('片段已删除，时间线与其他轨道位置保持不变') }
  const copySelection = () => { if (!selectedClip) return setStatus('请先点击需要复制的音频片段'); const copied = renderBuffer(selectedClip.buffer, { start: selectedClip.offset, end: selectedClip.offset + selectedClip.duration, gain: 1, fadeIn: 0, fadeOut: 0 }); setAudioClipboard(copied); setStatus(`已复制片段：${formatTime(selectedClip.duration, true)}`) }
  const cutSelection = () => { if (!selectedClip) return setStatus('请先点击需要剪切的音频片段'); remember(); const copied = renderBuffer(selectedClip.buffer, { start: selectedClip.offset, end: selectedClip.offset + selectedClip.duration, gain: 1, fadeIn: 0, fadeOut: 0 }); setAudioClipboard(copied); trackStore.deleteClip(trackStore.activeId, selectedClip.id); setSelectedClipId(''); setStatus('片段已剪切，原时间位置保留空白') }
  ...
  return <div className="app" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); loadFile(e.dataTransfer.files[0]) }}>
    ...
    <main>
      ...
      <section className="workspace">
        ...
        <div className={`track ${trackStore.tracks[0].expanded ? 'expanded' : ''}`}><TrackControls track={trackStore.tracks[0]} active={trackStore.activeId === 'track-1'} onActivate={() => trackStore.setActiveId('track-1')} onChange={patch => trackStore.updateTrack('track-1', patch)}/><div className="track-canvas">{buffer ? <><TrackClipLane track={trackStore.tracks[0]} timelineDuration={timelineDuration} currentTime={currentTime} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId('track-1'); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (trackStore.tracks[0].playbackRate * trackStore.tracks[0].clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/><ClipControls track={trackStore.tracks[0]} onChange={patch => trackStore.updateTrack('track-1', patch)} onApplyPitch={() => applyPitch('track-1')}/></> : <button className="empty" onClick={() => requestImport('main')}><Upload/><strong>导入音频，开始创作</strong><span>视频可从右侧监看区导入</span></button>}</div></div>
        <div className={`track video-audio-track ${trackStore.tracks[1].expanded ? 'expanded' : ''}`}><TrackControls track={trackStore.tracks[1]} active={trackStore.activeId === 'video-1'} onActivate={() => trackStore.setActiveId('video-1')} onChange={patch => trackStore.updateTrack('video-1', patch)} onExtract={trackStore.tracks[1].buffer ? extractVideoToMain : undefined}/><div className="track-canvas">{trackStore.tracks[1].buffer ? <><TrackClipLane track={trackStore.tracks[1]} timelineDuration={timelineDuration} currentTime={currentTime} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId('video-1'); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (trackStore.tracks[1].playbackRate * trackStore.tracks[1].clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/><ClipControls track={trackStore.tracks[1]} onChange={patch => trackStore.updateTrack('video-1', patch)} onApplyPitch={() => applyPitch('video-1')}/></> : <button className="video-empty" onClick={() => requestImport('video-1')}><Video/><span>导入视频，原声显示在此轨</span></button>}</div></div>
        <div className={`track ${trackStore.tracks[2].expanded ? 'expanded' : ''}`}><TrackControls track={trackStore.tracks[2]} active={trackStore.activeId === 'track-3'} onActivate={() => trackStore.setActiveId('track-3')} onChange={patch => trackStore.updateTrack('track-3', patch)}/><div className="track-canvas">{trackStore.tracks[2].buffer ? <><TrackClipLane track={trackStore.tracks[2]} timelineDuration={timelineDuration} currentTime={currentTime} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId('track-3'); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (trackStore.tracks[2].playbackRate * trackStore.tracks[2].clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/><ClipControls track={trackStore.tracks[2]} onChange={patch => trackStore.updateTrack('track-3', patch)} onApplyPitch={() => applyPitch('track-3')}/></> : <button className="video-empty" onClick={() => requestImport('music')}><Music2/><span>导入背景音乐</span></button>}</div></div>
        {trackStore.tracks.slice(3).map(track => <div className={`track extra-track ${track.expanded ? 'expanded' : ''}`} key={track.id}><TrackControls track={track} active={trackStore.activeId === track.id} onActivate={() => trackStore.setActiveId(track.id)} onChange={patch => trackStore.updateTrack(track.id, patch)} onDelete={() => trackStore.deleteTrack(track.id)}/><div className="track-canvas">{track.buffer ? <><TrackClipLane track={track} timelineDuration={timelineDuration} currentTime={currentTime} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId(track.id); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (track.playbackRate * track.clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/><ClipControls track={track} onChange={patch => trackStore.updateTrack(track.id, patch)} onApplyPitch={() => applyPitch(track.id)}/></> : <button className="video-empty" onClick={() => requestImport(track.id)}><Upload/><span>导入音频</span></button>}</div></div>)}
