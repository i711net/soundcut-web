...
export default function App() {
  ...
  const cutSelection = () => {
    ...
    const rate = selectedClip.playbackRate * trackStore.activeTrack.playbackRate * trackStore.activeTrack.clipPlaybackRate, clipStart = selectedClip.start, clipEnd = clipStart + selectedClip.duration / rate
    const hasDraggedRange = range.timelineTo - range.timelineFrom > .01 && (range.timelineFrom > clipStart + .01 || range.timelineTo < clipEnd - .01)
    remember()
    if (hasDraggedRange) {
      const copied = renderBuffer(selectedClip.buffer, { start: range.sourceFrom, end: range.sourceTo, gain: 1, fadeIn: 0, fadeOut: 0 })
      setAudioClipboard(copied); trackStore.splitClipRange(trackStore.activeId, selectedClip.id, range.timelineFrom, range.timelineTo)
      setStatus('已在选区左右边界切成独立片段，可分别移动或编辑')
    } else {
      const at = currentTimeRef.current
      if (at <= clipStart + .01 || at >= clipEnd - .01) return setStatus('请把红色播放头拖到所选片段中间，再点击剪切')
      trackStore.splitClip(trackStore.activeId, selectedClip.id, at); setStatus(`已在 ${formatTime(at, true)} 切成左右两个片段`)
    }
    setSelectedClipId('')
  ...
  const moveTimelineClip = (clipId: string, start: number, targetTrackId: string) => {
    ...
    if (snapEnabled && clip && targetTrack) {
      const length = clip.duration / (targetTrack.playbackRate * targetTrack.clipPlaybackRate * clip.playbackRate)
      const candidates = [0, ...targetTrack.clips.filter(item => item.id !== clipId).flatMap(item => { const end = item.start + item.duration / (targetTrack.playbackRate * targetTrack.clipPlaybackRate * item.playbackRate); return [end + snapGap, item.start - length - snapGap] }).filter(value => value >= 0)]
      ...
      if (nearest !== null) { finalStart = nearest; snapped = true }
    }
    if (clip && targetTrack) {
      const targetRate = targetTrack.playbackRate * targetTrack.clipPlaybackRate, length = clip.duration / (targetRate * clip.playbackRate)
      const others = targetTrack.clips.filter(item => item.id !== clipId).map(item => ({ start: item.start, end: item.start + item.duration / (targetRate * item.playbackRate) }))
      const collides = (at: number) => others.some(item => at < item.end - .001 && at + length > item.start + .001)
      if (collides(finalStart)) {
        const safeCandidates = [0, ...others.flatMap(item => [item.end + (snapEnabled ? snapGap : 0), item.start - length - (snapEnabled ? snapGap : 0)])].filter(at => at >= 0 && !collides(at))
        const nearestSafe = safeCandidates.reduce<number | null>((best, at) => best === null || Math.abs(at - start) < Math.abs(best - start) ? at : best, null)
        if (nearestSafe !== null) { finalStart = nearestSafe; snapped = snapEnabled }
      }
    }
  ...
  return <div className="app" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); void openMediaToClipboard(e.dataTransfer.files[0]) }}>
    ...
    <main>
      ...
      <section className="workspace">
        ...
        <div className={`track video-audio-track ${trackStore.tracks[1].expanded ? 'expanded' : ''}`}><TrackControls track={trackStore.tracks[1]} active={trackStore.activeId === 'video-1'} onActivate={() => trackStore.setActiveId('video-1')} onChange={patch => trackStore.updateTrack('video-1', patch)} onApplyPitch={() => applyPitch('video-1')} onExtract={trackStore.tracks[1].buffer ? extractVideoToMain : undefined}/><div className="track-canvas">{trackStore.tracks[1].buffer ? <><TrackClipLane track={trackStore.tracks[1]} timelineDuration={timelineDuration} tool={timelineTool} selection={selection} onSelection={setSelection} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId('video-1'); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (trackStore.tracks[1].playbackRate * trackStore.tracks[1].clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/></> : <div className="video-empty" data-track-id="video-1"><span>空音轨 · 把片段拖到这里</span></div>}</div></div>
        <div className={`track ${trackStore.tracks[2].expanded ? 'expanded' : ''}`}><TrackControls track={trackStore.tracks[2]} active={trackStore.activeId === 'track-3'} onActivate={() => trackStore.setActiveId('track-3')} onChange={patch => trackStore.updateTrack('track-3', patch)} onApplyPitch={() => applyPitch('track-3')}/><div className="track-canvas">{trackStore.tracks[2].buffer ? <><TrackClipLane track={trackStore.tracks[2]} timelineDuration={timelineDuration} tool={timelineTool} selection={selection} onSelection={setSelection} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId('track-3'); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (trackStore.tracks[2].playbackRate * trackStore.tracks[2].clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/></> : <div className="video-empty" data-track-id="track-3"><span>空音轨 · 把片段拖到这里</span></div>}</div></div>
        {trackStore.tracks.slice(3).map(track => <div className={`track extra-track ${track.expanded ? 'expanded' : ''}`} key={track.id}><TrackControls track={track} active={trackStore.activeId === track.id} onActivate={() => trackStore.setActiveId(track.id)} onChange={patch => trackStore.updateTrack(track.id, patch)} onApplyPitch={() => applyPitch(track.id)} onDelete={() => trackStore.deleteTrack(track.id)}/><div className="track-canvas">{track.buffer ? <><TrackClipLane track={track} timelineDuration={timelineDuration} tool={timelineTool} selection={selection} onSelection={setSelection} selectedClipId={selectedClipId} onSelect={clip => { trackStore.setActiveId(track.id); setSelectedClipId(clip.id); setSelection([clip.start, clip.start + clip.duration / (track.playbackRate * track.clipPlaybackRate * clip.playbackRate)]) }} onMove={moveTimelineClip} onTrim={trimTimelineClip} onSeek={seek}/></> : <div className="video-empty" data-track-id={track.id}><span>空音轨 · 把片段拖到这里</span></div>}</div></div>)}
