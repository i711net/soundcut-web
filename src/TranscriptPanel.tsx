import { Cloud, Download, Languages, LoaderCircle, Sparkles } from 'lucide-react'
...
export default function TranscriptPanel(props: Props) {
  ...
  return <div className="transcript-panel">
    ...
    {segments.length > 0 ? <>
      ...
      <div className="transcript-downloads"><span>导出字幕</span>{(['txt','srt','vtt'] as const).map(format => <button key={format} onClick={() => downloadTranscript(props.fileName, segments, format)}><Download/>{format.toUpperCase()}</button>)}</div>
